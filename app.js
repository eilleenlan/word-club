(() => {
  ...
  function renderUnits() {
    ...
    $('book-label').textContent = `${scope.gradeName(grade)}${grade === 1 ? ' · My School — Book 1' : grade === 2 ? ' · All About Me — Book 2' : grade === 3 ? ' · My Neighborhood — Book 3' : grade === 4 ? ' · My Country — Book 4' : grade === 5 ? ' · Our World — Book 5' : grade === 6 ? ' · Our Planet — Book 6' : currentUnits.length ? ' · 課本單字' : ' · 本年級單字尚未加入'}`;
